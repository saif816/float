"""
Inference Stage 2 — CPU/GPU compatible
Change from original: auto-detects device, runs on CPU if no CUDA available.
"""
...
# ── Auto-detect device ─────────────────────────────────────────────
# Original hardcoded GPU rank=0. This detects CPU/GPU automatically.
DEVICE = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')
print(f"[FLOAT] Running on device: {DEVICE}")
...
class DataProcessor:
    def __init__(self, opt):
        ...
        self.fa = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, flip_input=False, device='cuda' if torch.cuda.is_available() else 'cpu')
...
class InferenceAgent:
    def __init__(self, opt):
        # Only call cuda.empty_cache() if CUDA is actually available
        ...
        with torch.no_grad():
            for model_name, model_param in self.G.named_parameters():
                if model_name in state_dict:
                    model_param.copy_(state_dict[model_name].to(device))  # ← was: .to(rank) int
...
if __name__ == '__main__':
    ...
    if opt.res_video_path is None:
        ...
        res_video_path = os.path.join(
            ...
            % (call_time, video_name, audio_name, opt.nfe, opt.seed,
               opt.a_cfg_scale, opt.e_cfg_scale, opt.emo)
        )
    else:
